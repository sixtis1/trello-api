...
export class CardController {
    ...
    @ApiResponse({ status: 400, description: 'A card with this name already exists.' })
    ...
    @ApiParam({ name: 'cardId', type: Number, description: 'ID of the card to update' })
    ...
    async update_card(
        ...
        return await this.cardService.update_card(request.params.columnId, cardId, cardDTO);
    }

    @ApiOperation({ summary: 'Delete a card by ID' })
    @ApiParam({ name: 'cardId', type: Number, description: 'ID of the card to delete' })
