...
export class CommentService {
  ...
  async find_comment_by_id_and_card_id(
    cardId: number,
    dto: CommentDTO,
  ...
  async create_comment(cardId: number, dto: CommentDTO) {
    if (!(await this.check_exists_card(cardId)))
      throw new NotFoundException(errors.CARD_DOESNT_EXISTS);
  ...
  async update_comment(cardId: number, commentId: number, dto: CommentDTO) {
    ...
    if (dto.content == comment.content)
      throw new BadRequestException(errors.COMMENT_ALREADY_EXISTS);
