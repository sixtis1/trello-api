...
export class ColumnService {
  constructor(
    ...
    private readonly columnRepository: typeof ColumnModel,
  ) {}

  async find_column_by_id_and_user(
    columnId: number,
    userId: number,
  ...
  async find_column_by_name_and_user_id(
    user: User,
    dto: ColumnDTO,
  ): Promise<any> {
  ...
  async create_column(user: User, dto: ColumnDTO) {
    const existingColumn = await this.find_column_by_name_and_user_id(
      user,
      dto,
    );
    if (existingColumn)
      throw new BadRequestException(errors.COLUMN_ALREADY_EXISTS);
